...
<body class="bg-gray-100 font-poppins">
                ...
                @if(!empty($cartItems))
                    <table class="min-w-full table-auto border-collapse mb-6">
                        <thead class="bg-gray-200">
                            <tr>
                                <th class="px-4 py-2 text-left font-semibold text-gray-600">Social Publisher Name</th>
                                <th class="px-4 py-2 text-left font-semibold text-gray-600">Social Media Channel URL</th>
                                <th class="px-4 py-2 text-left font-semibold text-gray-600">Followers/Members</th>
    ...
    @foreach($cartItems as $index => $item)
        @if(isset($item['id']))
            ...
            <tr class="border-b">
                ...
                <td class="px-4 py-4 text-blue-500 underline">
                    <a href="{{ $item['url'] }}">{{ $item['url'] }}</a>
                </td>
                <td class="px-4 py-4 text-gray-600">{{ $item['followers_or_members'] }}</td>
                <td class="px-4 py-4 text-gray-600">{{ $item['influencer_category'] }}</td>
                <td class="px-4 py-4 text-gray-600">{{ $item['influencer_type'] }}</td>
    ...
    @endforeach
                    ...
                    <a href="{{ route('checkout.index') }}" class="block w-full text-center bg-green-500 text-white py-3 rounded-lg font-semibold hover:bg-green-600 transition mt-4">Proceed to Checkout</a>
                        ...
                        @foreach($cartItems as $item)
                            @if(isset($item['website_name']))
                                <div class="border p-4 rounded-lg">
                                    ...
                                    <p><strong>Followers/Members:</strong> {{ $item['followers_or_members'] }}</p>
                                    <p><strong>Category:</strong> {{ $item['influencer_category'] }}</p>
                                    <p><strong>Type:</strong> {{ $item['influencer_type'] }}</p>
