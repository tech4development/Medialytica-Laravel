...
<body class="bg-gray-100">
    <div class="container mx-auto p-6">
        ...
        @if($orderItems->isEmpty())
            <p class="text-gray-600">Your checkout is empty. <a href="{{ route('guest.page') }}" class="text-blue-500 hover:underline">Back to Social Media Channels</a></p>
        @else
            @if ($errors->any())
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="overflow-x-auto border border-gray-300 rounded-md">
                        <table class="min-w-full table-auto border-collapse">
                            <thead class="bg-blue-600 text-white">
                                <tr>
                                    <th class="px-4 py-2 border border-gray-300 text-left text-xs font-medium uppercase">Channel Name</th>
                                    <th class="px-4 py-2 border border-gray-300 text-left text-xs font-medium uppercase">Channel URL</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($orderItems as $item)
                                    <tr>
                                        <td class="px-4 py-2 border border-gray-300 text-sm text-gray-900">{{ $item['name'] }}</td>
                ...
                <section class="bg-white shadow-md rounded-lg p-4">
                    ...
                    <form id="checkout-form" method="POST" action="{{ route('order.place') }}">
                        ...
                        @else
                            <a href="{{ route('socialadvertiserlogin') }}" class="inline-block px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg text-center hover:bg-blue-700">
                                Proceed to Checkout
                            </a>
                        @endauth

                        <a href="{{ route('socialpublisher') }}" class="inline-block px-6 py-3 bg-green-600 text-white font-semibold rounded-lg text-center hover:bg-green-700">Back to Cart</a>
                    </form>
                </section>
            </div>
        @endif
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            checkoutButton?.addEventListener('click', function (event) {
                ...
                if (selectedMethod === 'paypal') {
                    ...
                    checkoutForm.action = "{{ route('socialorder.place') }}";
                }
