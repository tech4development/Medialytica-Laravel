...
@section('title', 'Get Started Advertisers')
...
@section('content')
 <div class="main-body bg-white">
  ...
  <div
   ...
   <h1
    ...
    <span class="relative whitespace-nowrap text-orange-500 dark:text-orange-300">
     ...
     <span class="relative" style="font-size: 35px;">Get Started As An Advertiser</span>
  ...
  <div class="mx-auto max-w-screen-xl px-4 py-8 lg:px-6 lg:py-16">
   ...
   <!-- Content Section -->
   <div class="grid lg:grid-cols-2 lg:items-center lg:gap-4"> <!-- Reduced gap from 8 to 4 -->
    <div>
     <div class="mt-4 space-y-6"> <!-- Reduced vertical spacing from 12 to 6 -->
      <div class="flex space-x-2"> <!-- Added space-x-2 to reduce horizontal space between items -->
       <div class="flex-shrink-0">
        <div class="flex h-12 w-12 items-center justify-center rounded-md">
         <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
          stroke="#004466" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <path d="M5 12l5 5L19 7"></path>
         </svg>
        </div>
       </div>
       <div>
        <h4 class="mt-2 text-lg font-medium">Create a free social publisher account
        </h4>
       </div>
      </div>
      ...
      <div class="flex space-x-2">
       ...
       <div>
        <h4 class="mt-2 text-lg font-medium">Submit your details
      ...
      <div class="flex space-x-2">
       ...
       <div>
        <h4 class="mt-2 text-lg font-medium">Receive orders from advertisers
      ...
      <div class="flex space-x-2">
       ...
       <div>
        <h4 class="mt-2 text-lg font-medium">Post advertisers' content on your social media pages
      ...
      <div class="flex space-x-2">
       ...
       <div>
        <h4 class="mt-2 text-lg font-medium">Get paid within 24 hours
        </h4>
       </div>
      </div>
     </div>
    ...
    <div aria-hidden="true" class="mt-10 lg:mt-0">
     <!-- Form Card -->
     <div x-data="{ showRegistration: true }" class="flex flex-col rounded-xl border border-gray-300 p-4 sm:p-6 lg:p-8">
      <!-- Toggle Button -->
      <button @click="showRegistration = !showRegistration"
       class="mb-4 flex items-center justify-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-100">
       Login Here
      </button>

      <!-- Registration Form -->
      <div x-show="showRegistration" x-transition:enter="transition ease-out duration-300"
       x-transition:enter-start="opacity-0 translate-y-8" x-transition:enter-end="opacity-100 translate-y-0"
       x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0"
       x-transition:leave-end="opacity-0 translate-y-8">
       <h2 class="mb-8 text-xl font-semibold text-gray-800">Register</h2>
       <form method="POST" action="{{ route('register') }}" class="grid gap-4 md:grid-cols-2">
        @csrf
        <!-- Role -->
        <div class="md:col-span-2">
         <label for="role" class="mb-2 block text-sm font-medium text-gray-700">Select Role</label>
        ...
        <div class="mt-4 flex items-center justify-between md:col-span-2">
         ...
         <button type="submit"
          class="hover:bg-[#b78700]focus:ring-4 rounded-lg bg-[#004466] px-4 py-3 text-sm font-medium text-white focus:ring-[#b78700]">
          {{ __('Register') }}
         </button>
        </div>
        <!-- Google Button -->
        <button type="button"
         class="flex items-center justify-center rounded-lg bg-red-500 px-4 py-2 text-sm text-white hover:bg-red-600 focus:ring-4 focus:ring-red-300">
         <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="mr-2 h-5 w-5" viewBox="0 0 24 24">
          <path
           d="M22.54 11.29H12v3.51h5.91c-.86 3.1-3.5 4.86-6.59 4.86-4.01 0-7.41-3.3-7.41-7.36s3.4-7.36 7.41-7.36c1.86 0 3.5.64 4.8 1.84L18.63 4c-1.75-1.7-4.1-2.76-6.63-2.76C5.9 1.24 1.24 5.8 1.24 12s4.66 10.76 10.76 10.76c5.56 0 10.23-3.84 10.23-10.36a11.07 11.07 0 0 0-.13-1.11l.43-.5z" />
         </svg>
         Sign in with Google
        </button>
       </form>
      ...
      <div x-show="!showRegistration" x-transition:enter="transition ease-out duration-300"
       ...
       x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0"
       x-transition:leave-end="opacity-0 translate-y-8">
       <h2 class="mb-8 text-xl font-semibold text-gray-800">Login</h2>
       <form method="POST" action="{{ route('login') }}" class="space-y-4">
        ...
        <div>
         ...
         <input id="email" type="email" name="email" value="{{ old('email') }}"
          class="focus:border-primary focus:ring-primary mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring focus:ring-opacity-50"
        ...
        <div class="mt-4 flex items-center justify-between">
         ...
         <button type="submit"
          class="hoverbg-[#b78700] inline-flex items-center rounded-md border border-transparent bg-[#004466] px-4 py-2 text-sm font-medium text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-[#004466] focus:ring-offset-2">
          {{ __('Log in') }}
         </button>
   ...
   <div class="mb-8 mt-7 flex flex-col justify-center space-y-5 md:flex-row md:space-x-6 md:space-y-0 lg:space-x-10">
    <div class="relative md:w-5/12">
     ...
     <div class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transform">
      <h1 class="mb-3 text-center font-bold uppercase text-white lg:text-xl">Login</h1>
      <a href="{{ route('login') }}">
       <button
    ...
    <div class="relative md:w-5/12">
     ...
     <div class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transform">
      <h1 class="mb-3 text-center font-bold uppercase text-white lg:text-xl">Register</h1>
      <a href="{{ route('register') }}">
       <button
        class="lg:text-md w-full transform rounded-full border border-2 border-[#004466] bg-[#004466] px-6 py-3 text-xs font-medium text-white transition duration-300 ease-in-out hover:scale-110 focus:outline-none">Go
        to
        Register</button>
      </a>
     </div>
