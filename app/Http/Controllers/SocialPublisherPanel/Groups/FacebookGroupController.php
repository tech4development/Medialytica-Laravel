...
class FacebookGroupController extends Controller
{
    ...
    public function store(Request $request)
    {
         ...
         $validatedData = [
            ...
            'facebook_group_url' => $request->input('facebook_group_url'),
            'facebook_group_name' => $request->input('facebook_group_name'),
            'influencer_category' => $request->input('influencer_category'),
            'facebook_group_members' => $request->input('facebook_group_members') ?? 0, // Default to 0 if null
    ...
    $validatedData['price'] = max($prices);
          ...
          FacebookGroup::create($validatedData);

       // session()->put('success', 'Your publisher details have been successfully captured! We will be in touch with you shortly');

        // Redirect with success message
        return redirect()->route('socialpublisher.dashboard')->with('success', 'Facebook Profile Details created successfully.');
    ...
    public function index()
    {
        ...
        $facebookgroupspublishers = FacebookGroup::paginate(10);

        // Pass the data to the view
        return view('socialAdvertisers.pages.facebookgroup', compact('facebookgroupspublishers'));
    }
