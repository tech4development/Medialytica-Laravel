...
class WhatsappChannelController extends Controller
{
    ...
    {
        return view('socialpublisher.allgroups.whatsapp.channels.create');
    }
    ...
    public function store(Request $request)
    {
         $validatedData = $request->validate([
            ...
            'whatsapp_channel_name' => 'required|string|max:255',
            'whatsapp_channel_members' => 'nullable|numeric',
            'whatsapp_channel_url' => 'required|url|unique:scp_whatsapp_channels,whatsapp_channel_url|max:255',
            'influencer_category' => [
            'required',
            Rule::in([
        ...
        $validatedData = [
            ...
            'whatsapp_channel_name' => $request->input('whatsapp_channel_name'),
            'whatsapp_channel_url' => $request->input('whatsapp_channel_url'),
            'influencer_category' => $request->input('influencer_category'),
            'whatsapp_channel_members' => $request->input('whatsapp_channel_members'),
    ...
    $validatedData['price'] = max($prices);
        ...
        WhatsappChannels::create($validatedData);

         //return dd($validatedData);


       WhatsappChannels::create($request->all());

      //  return redirect()->route('socialpublisher.dashboard')->with('success', 'WhatsApp Channel created successfully.');
    }
     ...
     */
    public function edit(WhatsappChannels $whatsappChannel)
    {
        return view('socialpublisher.allgroups.whatsapp.channels.edit', compact('whatsappChannel'));
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, WhatsappChannels $whatsappChannel)
    {
        $request->validate([
            'name' => 'required|string|max:255',
            'email_address' => 'required|email|max:255',
            ...
            'whatapp_channel_url' => [
                ...
                'url',
                'max:255',
                Rule::unique('scp_whatsapp_channels', 'whatsapp_channel_url')->ignore($whatsappChannel->id)
            ],
            'influencer_category' => [
                'required',
                'string',
        ...
        $whatsappChannel->update($request->all());

        return redirect()->route('whatsappchannels.index')
    ...
    public function index()
    {
        ...
        $whatsappchannelspublishers = WhatsappChannel::paginate(10);

        // Pass the data to the view
        return view('socialAdvertisers.pages.whatsappchannel', compact('whatsappchannelspublishers'));
    }
