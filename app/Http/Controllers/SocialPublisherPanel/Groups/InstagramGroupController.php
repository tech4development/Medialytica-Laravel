...
use App\Models\InstagramGroups;
use Illuminate\Validation\Rule;

class InstagramGroupController extends Controller
{
    ...
    {
        return view('socialpublisher.allgroups.instagram.create');
    }
    ...
    public function store(Request $request)
    {
         $validatedData = $request->validate([
            ...
            'instagram_group_name' => 'required|string|max:255',
            'instagram_group_url' => 'required|url|unique:scp_instagram_groups,instagram_group_url|max:255',
            'instagram_group_members' => 'nullable|numeric',
            'influencer_category' => [
            'required',
            Rule::in([
         ...
         $validatedData = [
            ...
            'instagram_group_name' => $request->input('instagram_group_name'),
            'instagram_group_url' => $request->input('instagram_group_url'),
            'influencer_category' => $request->input('influencer_category'),
            'instagram_group_members' => $request->input('instagram_group_members'),
    ...
    $validatedData['price'] = max($prices);
        ...
        //InstagramGroups::create($request->all());

        $instagramPublisher =InstagramGroups::create($validatedData);

        //return dd($validatedData);

        return redirect()->route('socialpublisher.dashboard')->with('success', 'Instagram Group created successfully.');
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(InstagramGroups $instagramGroup)
    {
        return view('socialpublisher.allgroups.instagram.edit', compact('instagramGroup'));
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, InstagramGroups $instagramGroup)
    {
        $request->validate([
            'name' => 'required|string|max:255',
            'email_address' => 'required|email|max:255',
            ...
            'instagram_group_url' => [
                ...
                'url',
                'max:255',
                Rule::unique('scp_instagram_groups', 'instagram_group_url')->ignore($instagramGroup->id)
            ],
            'instagram_group_members' => 'nullable|numeric',
            'influencer_category' => [
                'required',
        ...
        $instagramGroup->update($request->all());

        return redirect()->route('instagramgroups.index')
                         ->with('success', 'Instagram Group updated successfully.');
    }
    ...
    public function index()
    {
        ...
        $instaramgroupspublishers = InstagramGroup::paginate(10);

        // Pass the data to the view
        return view('socialAdvertisers.pages.instagramgroup', compact('instagramgroupspublishers'));
    }
