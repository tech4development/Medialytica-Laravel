...
class TelegramGroupController extends Controller
{
    ...
    {
        return view('socialpublisher.allgroups.telegram.create');
    }
    ...
    public function store(Request $request)
    {
        $validatedData = $request->validate([
            ...
            'telegram_group_name' => 'required|string|max:255',
            'telegram_group_url' => 'required|url|unique:scp_telegram_groups,telegram_group_url|max:255',
            'telegram_group_members' => 'nullable|numeric',
            'influencer_category' => [
            'required',
            Rule::in([
            ...
            'telegram_group_name' => $request->input('telegram_group_name'),
            'telegram_group_url' => $request->input('telegram_group_url'),
            'influencer_category' => $request->input('influencer_category'),
            'telegram_group_members' => $request->input('telegram_group_members'),
    ...
    $validatedData['price'] = max($prices);
        ...
        //TelegramGroups::create($request->all());

         TelegramGroups::create($validatedData);

        //return dd($validatedData);

       return redirect()->route('socialpublisher.dashboard')->with('success', 'Telegram Group created successfully.');
    }
     ...
     */
    public function edit(TelegramGroups $telegramGroup)
    {
        return view('socialpublisher.allgroups.telegram.edit', compact('telegramGroup'));
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, TelegramGroups $telegramGroup)
    {
        $request->validate([
            'name' => 'required|string|max:255',
            'email_address' => 'required|email|max:255',
            ...
            'telegram_group_url' => [
                ...
                'url',
                'max:255',
                Rule::unique('scp_telegram_groups', 'telegram_group_url')->ignore($telegramGroup->id)
            ],
            'telegram_group_members' => 'nullable|numeric',
            'influencer_category' => [
                'required',
        ...
        return redirect()->route('telegramgroups.index')
     ...
     */
    public function destroy(TelegramGroups $telegramGroup)
    {
        $telegramGroup->delete();

        return redirect()->route('telegramgroups.index')
                         ->with('success', 'Telegram Group deleted successfully.');
    }
    ...
    public function index()
    {
        ...
        $telegramgroupspublishers = TelegramGroup::paginate(10);

        // Pass the data to the view
        return view('socialAdvertisers.pages.telegramgroup', compact('telegramgroupspublishers'));
    }
}
